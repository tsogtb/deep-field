import { Shape3D, Sphere, Box, Medium } from "./medium.js";
import { PhotonState } from "./photon_state.js";


export function computeNormalAtBoundary(
  shape: Shape3D,
  px: number,
  py: number,
  pz: number,
  ...
  if (shape instanceof Sphere) {
    ...
    if (absX >= absY && absX >= absZ) {
      normal[0] = Math.sign(dx); normal[1] = 0; normal[2] = 0;
    } else if (absY >= absX && absY >= absZ) {
      normal[0] = 0; normal[1] = Math.sign(dy); normal[2] = 0;
    } else {
      normal[0] = 0; normal[1] = 0; normal[2] = Math.sign(dz);
    }
  } else {
...
export function handleBoundary(
  ...
  if (Math.random() < reflectProbability) {
    ...
    if (to.absorption) state.energy[idx] *= Math.exp(-to.absorption);
  }
