import { WorldBounds } from "./world_bounds.js";
import { PhotonState } from "./photon_state.js";
import { MediumRegion, VACUUM_REGION, findRegionIndexAt } from "./medium.js";
import { computeNormalAtBoundary, handleBoundary } from "./boundary.js";

const tempNormal = new Float32Array(3);

export function stepPhotons(
  state: PhotonState,
  dt: number,
  worldMedia: MediumRegion[],
  bounds?: WorldBounds
): void {
  const { count, position, prevPosition, direction, speed = 299_792_458, alive } = state;
...
const index1 = findRegionIndexAt(
  position[idx], 
  position[idx+1], 
  position[idx+2], 
  worldMedia
